...
class DetailcompleteController {
    async DetailComplete ( req, res, next) {
        ...
        return res.render('detailComplete',{order:order,listAddress:listAddress,staffShip:staffShip});
    }

    async evaluate (req, res, next) {
        if(!req.user){
            req.flash('message', 'Vui lòng đăng nhập để đánh giá đơn hàng !!!');
            return res.redirect('/login');
        }
        const order = await Order.findById(req.params.id).populate('addressId').populate('staffShip').lean();

        await Order.findByIdAndUpdate(req.params.id,{$set: {evaluate:req.body.evaluateSelect,evaluateImage:req.body.image, evaluateContent:req.body.evaluate}})
...
module.exports = new DetailcompleteController;